...
def numpy_to_image(a: numpy.ndarray, iname: str) -> bpy.types.Image:
    ...
    # suffix as Blender seems to use the ".%03d" pattern too avoid creating duplicate ids.
    iname_59 = iname[:59]
    ...
    def find_image(name: str, width: int, heigh: int) -> Optional[bpy.types.Image]:
        ...
        for image in bpy.data.images:
            if image.name == name and image.size[0] == width and image.size[1] == height:
                return image
    ...
    if image is None:
        ...
        # If 'shorten_iname' id didn't exist previously, then
        # it should have been created without changing its name.
